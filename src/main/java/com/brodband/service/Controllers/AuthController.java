...
public class AuthController implements IAuthController {
    ...
    public ResponseEntity<BasicMessageDTO> signUp(@RequestBody SignUpRequestDTO signUpRequestDTO) {
        ...
        if(signUpRequestDTO.getRole().equalsIgnoreCase("technician") && !signUpRequestDTO.getMasterPassword().equals("India@123")) {
            return new ResponseEntity<>(new BasicMessageDTO(false, "Wrong Master Password"), HttpStatus.BAD_REQUEST);
        }
        ...
        final Optional<User> user = userRepository.findUserByEmail(signUpRequestDTO.getEmail());
        User _user = user.get();

        Address newAddress = new Address();
        newAddress.setFirstName(signUpRequestDTO.getFirstName());
        newAddress.setLastName(signUpRequestDTO.getLastName());
        newAddress.setUserID(_user.getUserId());
        newAddress.setEmail(signUpRequestDTO.getEmail());
        addressRepository.save(newAddress);

        return new ResponseEntity<>(new BasicMessageDTO(true, "User Registered"), HttpStatus.CREATED);
    }
}
